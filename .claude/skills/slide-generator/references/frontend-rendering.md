...
- Reserve the chrome area in the layout. Content should end above sources/meta, and the scaled stage should leave room for presenter controls on short laptop screens.
...
- Verify navigation state. URL hashes, overview cards, keyboard arrows and buttons must show the same slide.
- If the deck has in-browser editing, saved edits must be keyed by deck version/signature. Never restore saved text into changed slide structures by positional index alone.
...
- hidden slides that remain exposed to browser snapshots or screen readers,
- stale local edits that corrupt a newly generated deck.

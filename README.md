...
Generated eval `deck/` and `qa/` output is ignored by git and regenerated by `npm test`. The A/B report is a fixture-level proxy comparison, not proof that every future skill run beats every one-shot prompt.
...
| `npm run eval:ab -- evals/source-backed/hackathon-rubric-eval` | Compare the source-backed skill run against the frozen one-shot baseline fixture. |
...
| `evals/` | Source-backed eval inputs, frozen baseline fixtures, and durable `work/` artifacts. |

...
if (!target) {
  console.error("Usage: node scripts/build-deck.mjs <project-dir> [--init] [--render] [--export]");
  process.exit(2);
}
...
try {
  ...
  await writeFile(path.join(projectDir, "qa", "deck-build.json"), JSON.stringify({
    project: path.relative(repoRoot, projectDir),
    generated_at: new Date().toISOString(),
    mode: { init, render, export: exportDeck },
    status: "pass",
    steps,
    note: "This is a deterministic build runner. Agentic planning happens before this script writes or repairs artifacts."
  }, null, 2) + "\n");
  console.log(`deck build passed for ${path.relative(repoRoot, projectDir)}`);
} catch (error) {
  ...
  await writeFile(path.join(projectDir, "qa", "deck-build.json"), JSON.stringify({
    ...
    steps,
    error: error.message,
    note: "This is a deterministic build runner. Fix the failed artifact before rerunning."
  }, null, 2) + "\n").catch(() => {});
